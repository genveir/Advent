...
public static class Helper
{
    ...
    public static void InitAndUpdate<KeyType, ValueType>(this Dictionary<KeyType, ValueType> dict,
        KeyType key, Func<ValueType, ValueType> action, ValueType defaultValue = default)
    {
        ...
        char emptyChar = ' ', bool flipX = false, bool flipY = false)
    {
        ...
        bool xCondition(int x) => flipX ? x >= minX : x <= maxX;
        int xIncrement(int x) => flipX ? x - 1 : x + 1;

        var fromY = flipY ? minY : maxY;
        bool yCondition(int y) => flipY ? y <= maxY : y >= minY;
        int yIncrement(int y) => flipY ? y + 1 : y - 1;
